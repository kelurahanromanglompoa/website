...
@section('content')
    ...
    <div class="card card-custom m-3">
        ...
        <div class="card-body">
            <div class="col-12">
                <form class="form" action="{{route('postingans.update', ['slug' => $slug, 'uuid' => $uuid, 'uuid_item' => $data->uuid])}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="form-group col-12">
                        ...
                        <select name="is_headline" class="form-control @error('is_headline') is-invalid @enderror" required>
                            ...
                            <option value="0" {{ $data->is_headline == '0' ? 'selected' : ''}}>Bukan Headline</option>
                        </select>
                        @error('is_headline')
                            <span class="form-text text-danger">{{$message}}</span>
                        @enderror
