                    ...
                    <tbody>
                        @forelse($datas as $item)
                            <tr>
                                ...
                                <td>
                                    @if(auth()->user()->role_id == \App\Models\Role::SUPER_ADMIN)
                                        <a href="{{route('postingans.kategori.destroy', ['uuid' => $item->uuid])}}" class="badge badge-sm badge-danger" onclick="return confirm('Apakah Anda Yakin Menghapus Data {{$item->nama}} ?')">
                                            Hapus
                                        </a>
                                    @endif
                                </td>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Tambah Kategori Postingan</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <i aria-hidden="true" class="ki ki-close"></i>
                    </button>
                </div>
                <div class="modal-body">
                    <form class="form" action="{{route('postingans.kategori.store')}}" method="POST">
                        @csrf
